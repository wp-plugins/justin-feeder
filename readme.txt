=== JustIn Feeder ===
Contributors: Marcelismus
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=JHPNQ9N3YMPFY
Tags: feed, shortcode
Requires at least: 3.4
Tested up to: 3.5
Stable tag: 1.0.1
License: GPLv3
License URI: http://www.gnu.org/licenses/gpl-3.0

Via Shortcode Content in einen Artikel oder eine Seite einfügen, der nur wahlweise nur im Feed oder auch nicht im Feed angezeigt wird.


== Description ==

"JustIn Feeder" fügt zwei Shortcodes dem WYSIWYG-Editor ein, die den Inhalt wahlweise nur im Feed oder nur im Artikel (beziehungsweise auf der Seite) anzeigt.

= Einbindung =

Alle Stores in einem Shortcode integriert und können via Button im Editor von WordPress eingefügt werden. Der Aufbau des Shortcodes ist dabei immer der folgende:

[infeed]*content*[/infeed] => nur im Feed anzeigen

[notinfeed]*content*[/notinfeed] => nicht im Feed anzeigen

= Systemanforderungen =
* PHP ab 5.2.4
* WordPress ab 3.4

= Autor =
* [blogtogo.de](https://plus.google.com/110569673423509816572 "blogtogo.de")
* [hirngedoens.de](https://plus.google.com/110569673423509816572 "hirngedoens.de")
* [der-s.com](https://plus.google.com/110569673423509816572 "der-s.com")
* [Twitter](https://twitter.com/Marcelismus "Twitter")
* [Google+](https://plus.google.com/106153855794163421374/ "Google+")


= Requirements =
* PHP ab 5.2.4
* WordPress ab 3.4

= Installation =
1. Downloadpaket entpacken
2. Den Plugin-Ordner nach "/wp-content/plugins/" hochladen
3. Das Plugin in WordPress aktivieren


== Frequently Asked Questions ==
  
= Gibt es Vorraussetzungen für den Server? =
  Ja: Der Server muss mindestens auf PHP5 und WordPress 3.4 laufen.


== Changelog ==

= 1.0.1 =
* Codeoptimierung

= 1.0.0 =
* Code-Freeze